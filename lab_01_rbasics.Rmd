...
***Answer***<br />
```{r}
big_bin_draws <- rbinom(n = 2000000, size = 1000000, prob = 0.5)
```
...
cat("Mean:", mean(big_bin_draws_standardized))
cat("St. Deviation:",sd(big_bin_draws_standardized))
```
...
***Answer***<br />
The shape of the histogram appears to be a normal distribution. No, it is not suprising as the Central Limit Theorem states that independent random variables after being normalized tend to look like the normal distribution.
```{r}
...
***Answer*** <br />
Yes, the proportion is close to 0.05.
```{r}
cat("Proportion:",sum(big_bin_draws_standardized > 1.644854) / length(big_bin_draws_standardized))
```
...
***Answer*** <br />
Yes, this value is also very close to 0.05.
```{r}
normal_random <- rnorm(2000000)
cat("Probability > 1.644854:",sum(normal_random > 1.644854) / length(normal_random))
```
...
***Answer***<br />
We could not name three Justin Bieber songs, so do we win?
```{r}
huge_bin_draws <- rbinom(n = 100000000, size = 10000000, prob = 50*10^-8)
```
...
***Answer***<br />
These are the very close to the values that we expect as with so many trials the values should be closer to the theoretical. It seems like it took longer to generate huge_bin_draws than these values. 
```{r}
cat("Mean:", mean(huge_bin_draws))
cat("St. Deviation:",sd(huge_bin_draws))
```
...
***Answer***<br />
It was a little bit slower to calculate the median. This makes sense as calculating the median has more calculations than the mean.
```{r}
cat("Median:",median(huge_bin_draws))
```
...
***Answer***<br />
It seems like it took longer than generating `huge_bin_draws` itself. This isn't suprising as we have to find the log of 100,000,000 values and then find the median of that.
```{r}
...
***Answer***<br />
Yes, this is different because there is a much lower probability. The distribution below resembles a Poisson distribution more than a normal distribution. This isn't suprising as Possion distributions are for rare events within large amounts of observations.
```{r}
...
cat("Class:",class(big_bin_draws_list))
cat("Length:",length(big_bin_draws_list))
cat("1049th Element Equal:",big_bin_draws_list[1049] == big_bin_draws[1049])
```
...
***Answer***<br />
It did not take longer to execute than my code in Q5b. This is because lapply had to go row by row and call a functino calculate the standardized number. But in my code, it used one command to calculate the standardized numbers.
...
***Answer*** <br />
This code is slower because it calculates the mean and standard deviation every time the function is called. So the number of operations per function call just went up by two.
...
***Answer*** <br />
The list requires more memory. This is the case because lists are recursive and can have any data type, which makes them slower. This property is not important for sotring binomial draws as the data types are all the same.
```{r}
cat("big_bin_draws Storage:",object.size(big_bin_draws) / 1000000)
cat("big_bin_draws_list Storage:",object.size(big_bin_draws_list) / 1000000)
```
